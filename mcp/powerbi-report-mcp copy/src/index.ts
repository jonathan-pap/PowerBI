...
function parseFieldSpec(spec: {
  ...
  if (spec.type === "measure") {
    return measureRef(spec.entity, spec.property);
  }
  if (spec.type === "aggregation") {
    const func = AggregationFunction[spec.aggregation || "Sum"] ?? 0;
    return aggregationRef(spec.entity, spec.property, func);
  }
  return columnRef(spec.entity, spec.property);
}

// --- Helper: Build formatting props in PBIR literal format ---
function buildFormattingProps(properties: Record<string, string | number | boolean>): Record<string, unknown> {
...
function applyFormattingToTarget(
...
// --- Helper: Create a single visual and save it ---
interface VisualSpec {
  ...
  width?: number;
  height?: number;
  bindings?: Array<{ bucket: string; fields: Array<{ entity: string; property: string; type: "column" | "measure" | "aggregation"; aggregation?: string }> }>;
  autoFilters?: boolean;
...
function createAndSaveVisual(
  ...
  let sortDefinition: { sort: { field: FieldRef; direction: "Ascending" | "Descending" }[]; isDefaultSort?: boolean } | undefined;
  if (queryState.Category?.projections?.[0]) {
    sortDefinition = {
      sort: [{ field: JSON.parse(JSON.stringify(queryState.Category.projections[0].field)), direction: "Ascending" as const }],
      isDefaultSort: true,
    };
  }
  if (!sortDefinition && visualType === "slicer" && queryState.Values?.projections?.[0]) {
    sortDefinition = {
      sort: [{ field: JSON.parse(JSON.stringify(queryState.Values.projections[0].field)), direction: "Ascending" as const }],
    };
  }
  ...
  if (containerFormat && containerFormat.length > 0) {
    applyFormattingToTarget(visual.visual.visualContainerObjects as Record<string, unknown>, containerFormat);
  }
...
const THEME_PRESETS: Record<string, {
  containerFormat: Array<{ category: string; properties: Record<string, string | number | boolean> }>;
  chartVisualFormat?: Array<{ category: string; properties: Record<string, string | number | boolean> }>;
  slicerContainerFormat?: Array<{ category: string; properties: Record<string, string | number | boolean> }>;
  dataColors?: string[];
}> = {
  dark: {
    containerFormat: [
...
async function main() {
  ...
  const FieldSpecSchema = z.object({
    entity: z.string().describe("Table/entity name (e.g. 'financials')"),
    property: z.string().describe("Column or measure name (e.g. 'Gross Sales')"),
    type: z
  ...
  const FormatCategorySchema = z.object({
    category: z.string().describe("Formatting category"),
    properties: z.record(z.string(), z.union([z.string(), z.number(), z.boolean()])).describe("Property key-value pairs"),
  });
  ...
  const VisualSpecSchema = z.object({
    ...
    shapeType: z.enum(["rectangle", "rectangleRounded", "line", "tabCutCorner", "tabCutTopCorners", "tabRoundCorner", "tabRoundTopCorners"]).optional(),
    shapeRotation: z.number().optional().default(0),
    fillColor: z.string().optional().describe("Fill color for shapes (hex, e.g. '#D9D9D9'). Defaults to #D9D9D9 if not provided."),
    ...
    textAlign: z.enum(["left", "center", "right"]).optional().describe("Horizontal text alignment inside shape/textbox"),
    textSize: z.number().optional().describe("Font size in pt (e.g. 10)"),
    textBold: z.boolean().optional().describe("Bold text"),
    title: z.string().optional(),
    containerFormat: z.array(FormatCategorySchema).optional().describe("Inline container formatting (title, background, border, padding, dropShadow, visualHeader)"),
    visualFormat: z.array(FormatCategorySchema).optional().describe("Inline visual formatting (axes, legend, labels, lineStyles, etc.)"),
  ...
  server.tool(
    ...
    `Add one or more visuals to a page with optional inline formatting. Supports single mode (use top-level params) or batch mode (use 'visuals' array). Inline containerFormat, visualFormat, and dataColors eliminate the need for separate format_visual/set_datapoint_colors calls. IMPORTANT chart naming (Power BI convention): 'columnChart'=stacked column (use Series bucket for stack breakdown), 'barChart'=stacked bar (use Series bucket for stack breakdown), 'clusteredColumnChart'=clustered column, 'clusteredBarChart'=clustered bar, 'hundredPercentStackedColumnChart'=100% stacked column, 'hundredPercentStackedBarChart'=100% stacked bar. Visual types: ${Object.keys(VISUAL_BUCKETS).join(", ")}.`,
    {
      ...
      dataColors: z.array(DataColorSchema).optional().describe("Inline data point colors"),
      // --- Batch mode ---
      visuals: z.array(VisualSpecSchema).optional().describe("Array of visuals to add (batch mode). When provided, top-level visual params are ignored."),
    },
    async (params) => {
      const { pageId } = params;
  ...
  server.tool(
    ...
    "Update the data bindings of an existing visual. Replaces the query state entirely.",
    {
      pageId: z.string().describe("The page ID"),
      visualId: z.string().describe("The visual ID"),
  ...
  server.tool(
    ...
    "Set or update the title of a visual. Can set text, visibility, font, size, alignment.",
    {
      ...
      fontFamily: z.string().optional().describe("Font family (e.g. \"'Segoe UI', wf_segoe-ui_normal, helvetica, arial, sans-serif\")"),
    ...
    async ({ pageId, visualId, title, show, fontSize, fontFamily, alignment, titleWrap }) => {
      ...
      const existing = (visual.visual.visualContainerObjects as Record<string, unknown[]>).title;
      if (Array.isArray(existing) && existing.length > 0) {
        const existingProps = (existing[0] as { properties: Record<string, unknown> }).properties || {};
        (existing[0] as { properties: Record<string, unknown> }).properties = { ...existingProps, ...titleProps };
      } else {
        (visual.visual.visualContainerObjects as Record<string, unknown[]>).title = [{ properties: titleProps }];
      }
  ...
  server.tool(
    ...
    "Set data point colors for specific series/measures in a visual.",
    {
      ...
      defaultTransparency: z.number().optional().describe("Default transparency for all data points (0-100)"),
    },
  ...
  server.tool(
    ...
    "Duplicate an entire page with all its visuals to a new page",
    {
      ...
      displayName: z.string().optional().describe("Display name for the new page (defaults to 'Copy of <original>')"),
    },
    async ({ pageId, displayName }) => {
  ...
  server.tool(
    ...
    {
      pageId: z.string().describe("The page ID"),
      theme: z.enum(["dark", "light", "corporate", "blue-purple"]).describe("Theme preset name"),
      applyDataColors: z.boolean().optional().default(true).describe("Whether to apply theme data colors to chart visuals"),
    },
    async ({ pageId, theme, applyDataColors: applyColors }) => {
      ...
      for (const vid of visualIds) {
        ...
        if (vType === "textbox" || vType === "shape" || vType === "basicShape" || vType === "image") continue;
        ...
        if (!visual.visual.visualContainerObjects) visual.visual.visualContainerObjects = {};
        applyFormattingToTarget(visual.visual.visualContainerObjects as Record<string, unknown>, containerFmt);

        // Apply chart-specific visual formatting if available
        if (preset.chartVisualFormat && chartTypes.has(vType)) {
          if (!visual.visual.objects) visual.visual.objects = {};
          applyFormattingToTarget(visual.visual.objects as Record<string, unknown>, preset.chartVisualFormat);
        }
